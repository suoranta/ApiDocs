Here you can find documentation of public APIs for my packages and SDKs. Reference assemblies will published in due course.

## Packages
- [Suoranta.EnterpriseLib](https://suoranta.github.io/ApiDocs/Suoranta.EnterpriseLib/): Applications hosts for ASP.NET MVC and CLI applications, LINQ query objects, WebAPI controllers, Entity-DTO-Entity mapping, mock data generation, other functionalities related to Entity Framework and ASP.NET MVC enterprise applications.
- [Suoranta.NaturalLanguage](https://suoranta.github.io/ApiDocs/Suoranta.NaturalLanguage/): Advanced text pattern searching in documents and images, transliteration, other functionalities related to natural language processing.
- [Suoranta.Speech](https://suoranta.github.io/ApiDocs/Suoranta.Speech/): Voice commands for WinForms and other .NET apps.
- [Suoranta.WebApp](https://suoranta.github.io/ApiDocs/Suoranta.WebApp/): Upgraded Razor infrastructure, JavaScript support modules (not included in this documentation), KendoUI extensions

## SDKs
- [Sharpshooter.SDK](https://suoranta.github.io/ApiDocs/Sharpshooter.SDK/): (coming soon)
- [SharpTrader.SDK](https://suoranta.github.io/ApiDocs/SharpTrader.SDK/): (coming soon)
